...
with ui.dialog() as help_dialog:
    with ui.card().classes("w-96"):
        ...
        ui.button("Close", on_click=help_dialog.close).props("flat").classes(
            "ml-auto text-[#EEEDF0]"
        )

# --- HEADER BUTTONS ---
with ui.row().classes("absolute top-2 right-2 items-center"):
    ui.button(icon="help", on_click=help_dialog.open).props("round flat").classes(
        "text-[#EEEDF0]"
    )
    ui.button(icon="code", on_click=lambda: right_drawer.toggle()).props(
        "round flat"
    ).classes("text-[#EEEDF0]")
...
with ui.column().classes("w-full max-w-xl mx-auto items-center p-8 gap-4 mt-12"):
    ...
    auto_download_toggle = (
        ui.switch("Auto Download artists' future albums", value=False)
        .props("color=pink-4")
        .classes("text-[#EEEDF0] mt-2")
    )

    # SUBMIT BUTTON
    ui.button("Submit", on_click=lambda: on_submit()).style(
